---
title: 쿠버네티스 클러스터 내부의 Service 동작 원리 분석 (작성중)
author: jemlog
...
위에서 Service가 생성되고 endpoint와 매핑되는 과정을 살펴봤다. 작업이 완료된 후에 api-server는 Kube-Proxy에게 해당 변경사항을 전파한다. Kube-Proxy는 이 변경사항들을 NAT Rule로 변환해서 적용한다. 이 NAT Rule들은 요청되는 Service IP를 파드 IP로 변환하는 역할을 한다.

만약 파드의 IP를 나타내는 endpoint는 설정했는데, 실제 파드가 Unhealthy 상태라면 어떻게 될까? 서비스가 요청을 로드밸런싱해도 파드가 죽었기 때문에 트래픽을 
처리하지 못할 것이다. Kube-Proxy에게 파드들의 상태를 알려주는 작업이 필요한 것이다. 이 역할을 Kubelet이 수행한다. 

Kubelet은 쿠버네티스의 Node Agent로써 Endpoint 파드들의 Health를 지속적으로 모니터링하다가 UnHealthy 파드가 생기면 api-server로 전파한다.
api-server는 이 정보를 Kube-Proxy에게 전파한다. Kubectl 통해서 yaml 기반 명령 클러스터에 적용. Yaml 명령 api-server 갔다가 kubelet으로 간다. kubelet이 yaml 통해서 파드 생성하고, 지속적으로 실행 잘 되는지 확인.

kube-proxy는 클러스터 내의 모든 노드에 존재하고 Pod와 Service간 통신에 필요한 각종 필터링과 NAT를 수행해준다. 이때 Kube-Proxy는 리눅스의 iptables와 netfilter를 사용한다.

Kube-Proxy의 mode에는 userspace, iptables, ipvs가 있다.

### Userspace Mode

userspace 모드에서는 netfilter를 통해 들어오는 service 트래픽을 kube-Proxy로 라우팅한다. kube-proxy가 요청들어온 트래픽을 실제 server pod의 ip:port로 변환해서 eth0한테 전달한다. 기존에 kube-proxy가 실질적으로 proxy 역할을 할때는 kube-proxy 자체가 단일 장애 지점이 될 수 있었기 때문에 안정성이 떨어졌다. 
...
iptables은 기본적으로 방화벽을 위해 탄생한 기능이다. 쿠버네티스 환경에서 수많은 Service와 파드가
iptables 모드는 table을 탐색할때 Sequence Search를 사용한다. 이 순차 알고리즘은 Service와 Endpoint 개수가 늘어나면 O(n)의 시간복잡도로 인해 Lookup 횟수가 증가한다. 또한 iptables는 별도의 로드밸런싱 알고리즘을 제공하지 않고 random equal-cost way 방식만을 사용한다.

### IPVS Mode

iptables를 개선하기위해 등장한 ipvs에 대해 알아보자. ipvs는 로드밸런싱을 위해 특별히 디자인된 리눅스 기능이다. ipvs 모드에서는 Kube-Proxy가 iptables 대신 ipvs에 Rule을 삽입한다. ipvssms 내부적으로 해시를 사용해서 시간복잡도 O(1)의 최적화된 Lookup 알고리즘을 사용한다. 
따라서 내부적으로 Rule이 많아져도 항상 상수 시간의 성능을 보여준다는 장점이 있다. 또한 ipvs는 Round Robin, Least Connection 등의 다양한 로드밸런싱 알고리즘을 제공한다. 

이런 많은 장점이 존재하지만, ipvs는 모든 리눅스 시스템에 아직 존재하지는 않는다. 반면 iptables는 모든 리눅스 시스템의 핵심 특징이고, 
Service 개수가 엄청나게 많지 않다면 iptables 모드도 좋은 성능을 보여준다.

이번 포스팅에서는 iptables Mode에 집중해서 알아보도록 하자.

## iptables & netfilter

iptables는 커널의 패킷 필터링 프레임워크인 netfilter와 함께 사용되는 유명한 방화벽이다.

Iptables 방화벽은 리눅스 커널의 네트워킹 스택의 패킷 필터링 훅들과 상호 소통하며 동작한다.

이 커널 훅들은 netfilter 프레임워크로 알려져있다.

네트워킹 레이어를 지나가는 모든 패킷들은 이 훅들을 트리거한다, 그래서 프로그램들이 트래픽과 중요한 순간에 상호 소통하도록 만든다.

결국 패킷이 네트워킹 스택을 지나가면서 netfilter hook들을 트리거 하는 거구나.
...
커널 모듈들은 순서 제공해야 한다. 훅이 호출되면 어떤 순서로 호출될지를 위한. 여러개의 모듈이 훅들에 연결되도록 해준다. 각각의 모듈들은 호출되고 netfilter에게 결정을 반환할 것이다 패킷이 무엇을 해야할지.

IPtables 안에서 룰들은 분리된 chain에 소속된다. 테이블들은 그것들이 들고있는 룰의 보통의 목적에 의해 정의되지만, 체인들은 netfilter 훅을 대표한다. 체인은 룰이 언제 평가될지를 결정한다.

Rule에 Match되면 Targets가 된다. 이때 두가지 있음.
Terminating: 체인에서의 평가를 종료하고 netfilter hook에게 통제를 돌려준다. 결과에 따라서, 훅은 패킷을 드랍하거나 다음 스테이지로 패킷을 허용한다.
Non-terminating : 마지막에는 결국 종료결과를 줘야 하지만, 그 전에 수행된다.
Jump target은 매칭되면 추가적인 처리를 위해 다른 체인으로 이동한다. iptables는 관리자가 그들의 체인을 만들수 있도록 한다.
룰 들은 user-defined chain에 있을 수 있다. User-defied chain은 오직 jumping에 의해서 도달 가능하다. 그것들은 스스로 netfilter의 hook에 등록되지 않는다.

conntrack은 iptable이 진행되고 있는 커넥션의 맥락에서 결정하게 해준다. 커넥션 트래픽은 iptables가 stateful 연산하게 해준다.

Connection tracking은 packet이 네트워킹 스택 방문후 매우 빠르게 적용된다. Raw table chain과 몇몇 sanity 체크는 커넥션과 패킷을 연관짓기 전에 수행하는 오직 하나의 로직이다.

시스템은 존재하는 커넥션들과 패킷을 검사한다.

iptables로 다시 응답 돌아왔을때 변환해주는건 iptables의 conntrack 덕분이구나..!


## iptables를 사용한 Service 동작 원리

가장 중요한 chain으로는 `KUBE-SERVICE`, `KUBE-SVC-*` 그리고 `KUBE-SEP-*`가 있다.
...
쿠버네티스는 패킷 필터링이랑 NAT를 수행하기 위해 iptables에 `KUBE-SERVICES`라는 사용자 정의 chain을 생성한다. 이 chain을 추가하면 모든 PREROUTING과 OUTPUT 트래픽이 KUBE-SERVICES로 보내진다.

그렇다면 Service에서 정확하게 iptables가 어떻게 사용되는지 따라가보자.
...
처음 패킷이 들어왔을때 발동되는 nf_prerouting 훅에 연결된 PREROUTING Chain을 검색하면 연관된 Chain으로 여러가지가 나온다. 현재 Docker 기반의 minikube 환경에서 진행하고 있기 때문에 Docker와 관련된 Chain들도 등장한다. 여기서 살펴봐야할 Chain은 `KUBE-SERVICES`이다.
...
target     prot opt source               destination         
...
KUBE-SVC-V2OKYYMBY3REGZOG  tcp  --  anywhere             10.97.76.113         /* default/nginx-service cluster IP */ tcp dpt:http
...
```
`KUBE-SERVICES` 체인은 `KUBE-SVC-*` 체인과 연결되있다. 해당 체인은 로드밸런서의 역할을 하며 연결된 `KUBE-SEP-*`들로 트래픽을 전달한다.

```shell
$ iptables -t nat -L KUBE-SVC-V2OKYYMBY3REGZOG 
Chain KUBE-SVC-V2OKYYMBY3REGZOG (2 references)
target     prot opt source               destination         
KUBE-MARK-MASQ  tcp  -- !10.244.0.0/16        10.97.76.113         /* default/nginx-service cluster IP */ tcp dpt:http
KUBE-SEP-MUU44H3RRYQLTQ3I  all  --  anywhere             anywhere             /* default/nginx-service -> 10.244.0.67:80 */ statistic mode random probability 0.33333333349
KUBE-SEP-UGFU6IF6MA43DDMH  all  --  anywhere             anywhere             /* default/nginx-service -> 10.244.0.68:80 */ statistic mode random probability 0.50000000000
KUBE-SEP-JEOHPTUTGHFNQFB4  all  --  anywhere             anywhere             /* default/nginx-service -> 10.244.0.69:80 */
```
`KUBE-SEP-*`를 자세히 보면 DNAT 항목이 있고 주석에는 `10.244.0.67:80`으로 트래픽을 전달한다는 내용이 있다. 우리가 찾고자 했던 ClusterIP를 파드 IP로 변경시키는 부분이 여기서 수행이 되는 것이다. 

```shell
$ iptables -t nat -L KUBE-SEP-MUU44H3RRYQLTQ3I
Chain KUBE-SEP-MUU44H3RRYQLTQ3I (1 references)
target     prot opt source               destination         
KUBE-MARK-MASQ  all  --  10.244.0.67          anywhere             /* default/nginx-service */
DNAT       tcp  --  anywhere             anywhere             /* default/nginx-service */ tcp to:10.244.0.67:80
```
파드의 endpoint를 조회하면 `10.244.0.67:80`이 존재하는걸 알 수 있다.

```shell
$ kubectl get ep nginx-service
NAME            ENDPOINTS                                      AGE
nginx-service   10.244.0.67:80,10.244.0.68:80,10.244.0.69:80   15m
```
