...
## 락 구현 방식 선택
동시성 문제를 해결할 수 있는 여러가지 방법이 있지만, 저희는 **분산 서버 환경**에서 **특정 메서드에 대한 진입을 통제**해야 하기 때문에 분산락을 적용할 것이고, 흔히 분산락 구현을 위해 사용하는 **MySQL 네임드락**과 **Redis 기반 분산락**을
비교해보겠습니다.

MySQL 네임드 락은 MySQL을 사용한다면 함께 사용할 수 있는 방식입니다. 하지만 MySQL에 트래픽이 몰리는 상황에서는 락으로 인한 부하가 성능 저하를 유발할 수 있습니다. 또한 디스크 I/O가 발생하기에 속도가 느린 편입니다.
