...
@Order(Ordered.LOWEST_PRECEDENCE - 1) // @Transaction보다 먼저 AOP가 호출
```
AOP의 Order를 가장 낮은 순위 -1로 설정했는데요. Spring AOP는 순서의 숫자가 작을수록 우선순위가 높습니다. 예를 들어 Order(2)보다 Order(1)이 우선순위가 높은 것이지요.
AOP는 Default 우선순위로 Ordered.LOWEST_PRECEDENCE, 즉 가장 낮은 순서를 제공합니다.
테스트를 해본 결과 `@Transactional`도 해당 default 우선순위를 제공하는걸로 보였습니다. 그 중에서 같은 우선순위를 가진다면 @Transactional이 더 먼저 적용되는걸 확인했습니다.

낙관적락에서 버전 충돌을 확인하는 시점은 언제일까요? 바로 **트랜잭션이 커밋**되는 순간입니다. 따라서 버전 충돌을 체크하는 AOP는 트랜잭션 외부에서 감싸는 구조를 가지고 있어야 합니다.
저는 이를 위해서 낙관적락 재시도의 우선순위를 더 높게 설정함으로써 해결했습니다.
