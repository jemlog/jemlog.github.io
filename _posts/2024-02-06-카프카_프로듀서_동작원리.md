---
title: 카프카 Producer 내부 동작 흐름 살펴보기 1
author: jemlog
...
지금까지 메세지를 직렬화하고 파티션을 결정하고 배치에 적재하는 과정까지를 살펴봤다. 이제 메세지를 전송하는 일만 남았는데, 해당 부분은 Sender라는 **별도의 스레드**를 통해 **비동기로 수행**하게 된다. 
다음 글에서는 Sender의 내부 동작 방식에 대해 다뤄보고자 한다.
