...
그렇다면 **분산락**과 **낙관적락** 중에서 비교를 통해 적절한 락 방식을 선택해보겠습니다.
## 낙관적 락과 분산락 성능 비교
...
## DB 쓰기락에 대한 조금 더 깊은 생각
사실 해당 댓글 좋아요 기능에 분산락을 적용하는게 **오버 엔지니어링**으로 다가올 수도 있습니다. 분산락이 정말 필요한 영역은 하나의 트랜잭션에서 **여러 테이블에 대한 락이 걸려서 데드락의 위험이 전파되는 경우**라고 생각합니다.
반면 현재 댓글 좋아요 기능은 댓글 테이블 하나에만 락이 걸리면 되서 DB 쓰기락으로도 충분히 처리가 가능하고, 실제 구현을 해서 테스트 해봤을때 정상적으로 동작하기도 했습니다.

만약 실무에서 팀원들과 어떤 락을 적용해야 할지 후보를 정하는 과정이였다면 저는 현재 상황에서는 **DB 쓰기락**을 선택할 것 같습니다. 현업에서는 **오버 엔지니어링을 방지**해서 **비용을 최소화** 하는게 중요하다고 생각하니깐요. 하지만 이번 프로젝트에서는 DB 쓰기락과 분산락 중 새로운 기술에 대한 경험에 초점을 두고 **분산락**을 선택했습니다.

분산락과 DB 쓰기락 모두 **직접 락을 획득하는 방식**인 만큼 낙관적락에 비해 **동시 요청이 많은 상황에서 좋은 성능을 발휘**한다는 공통점에 집중했다는 사실만 미리 인지해주시면 감사하겠습니다.
