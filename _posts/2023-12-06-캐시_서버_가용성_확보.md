---
title: 분산 캐시 서버 가용성 개선기
author: jemlog
date: 2023-12-05 00:20:00
categories: [Project]
tags: [Redis, Circuit Breaker, Spring]
pin: false
img_path: '/assets/img/'
---
...
체크하기 위해 레디스 클라이언트로 사용중인 `Lettuce`의 Configuration을 분석해봤습니다.
  ...
  circuitbreaker:
    configs:
      cacheCircuit:
        ...
        minimumNumberOfCalls: 3 # 판단을 위한 최소 요청 횟수
        slidingWindowSize: 5 # 슬라이딩 윈도우 사이즈
        failureRateThreshold: 60 # OPEN이라고 판단하는 실패율
        waitDurationInOpenState: 10s # OPEN 상태에 머물러 있는 시간
        permittedNumberOfCallsInHalfOpenState: 3 # HALF_OPEN 상태에서 상태 전이를 판단하는 요청 횟수
...
## Spring Actuator health check 설정을 통한 로드밸런싱 장애 방지
...
결론적으로 가용성을 보장하기 위해서는 redis에 대한 헬스체크를 제외해야 합니다.

```yaml
management.health.redis.enabled: false
```
`RedisHealthIndicator` 자체를 재정의할수도 있겠지만, yaml 파일로도 redis를 헬스체크할지 여부를 선택할 수 있어서 `false`로 변경했습니다.
