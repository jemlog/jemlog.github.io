...
결론적으로 가용성을 보장하기 위해서는 redis에 대한 헬스체크를 제외해야 합니다. 저는 여기서 간단하게 actuator의 헬스체크 API 대신 커스터마이징한 헬스체크 API를 만드는 것으로 대체했습니다.
...
```
로드 밸런서 헬스 체크의 목표는 서버 자체가 살아있는지를 확인하는 것이라 생각하기 때문에 **200 상태 코드**가 **정해진 응답시간 안에** 반환되는지 체크하는 것으로 충분하다 판단했습니다.
